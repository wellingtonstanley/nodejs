 ...
 * @internal
 */
function createLogPrefix(connectionId, type, name) {
    const parts = [];
    if (connectionId) {
...
/** @internal */
function createReceiver(ctx, consumerGroup, partitionId, eventPosition, options = {}) {
    ...
    const obj = {
        ...
        async connect({ abortSignal, timeoutInMs }) {
            ...
            try {
                ...
                state.authLoop = await withAuth(() => setupLink(ctx, name, address, obj, state, queue, eventPosition, logger$1, options, abortSignal), ctx, audience, timeoutInMs, logger$1, {
                    abortSignal,
                });
            }
            catch (err) {
                        ...
                        : new Promise((resolve, reject) => {
                            ...
                            obj // eslint-disable-line promise/catch-or-return
                                ...
                                if (addCredits(state.link, eventsToRetrieveCount) > 0) {
                                    return logger$1.verbose(`setting the wait timer for ${maxWaitTimeInSeconds} seconds`);
                                }
                                else
                                    return;
                            })
                                .then(() => waitForEvents(maxMessageCount, maxWaitTimeInSeconds * 1000, qReadIntervalInMs, queue, {
                                abortSignal,
                                cleanupBeforeAbort,
...
function addCredits(receiver, eventsToRetrieveCount) {
    var _a;
    const creditsToAdd = eventsToRetrieveCount - ((_a = receiver === null || receiver === void 0 ? void 0 : receiver.credit) !== null && _a !== void 0 ? _a : 0);
    if (creditsToAdd > 0) {
...
function createRheaOptions(name, address, obj, state, queue, eventPosition, logger, options) {
    const rheaOptions = {
        name,
        autoaccept: true,
        source: {
    ...
    if (typeof ownerLevel === "number") {
        rheaOptions.properties = {
            [coreAmqp.Constants.attachEpoch]: rheaPromise.types.wrap_long(ownerLevel),
        };
    }
    if (options.trackLastEnqueuedEventProperties) {
...
async function setupLink(ctx, name, address, obj, state, queue, eventPosition, logger, options, abortSignal) {
    const rheaOptions = createRheaOptions(name, address, obj, state, queue, eventPosition, logger, options);
...
class PartitionPump {
    ...
    _setOrReplaceReceiver(partitionId, lastSeenSequenceNumber) {
        ...
        this._receiver = createReceiver(this._context, this._partitionProcessor.consumerGroup, partitionId, currentEventPosition, {
